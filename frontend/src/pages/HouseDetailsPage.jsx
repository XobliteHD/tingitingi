...
export default function HouseDetailsPage({ t, language }) {
  ...
  const handleRecaptchaChange = (token) => {
    ...
    if (token) {
       setFormErrors(prev => { const n = {...prev}; delete n.recaptcha; return n; });
    }
  };
  ...
  const loadBookedDates = useCallback(async () => {
      ...
      try {
          ...
          toast.error(t('fetchBookedDatesError'))
      }
  }, [houseId, t]);
  ...
  const handleStartDateChange = (date) => { setStartDate(date); if (endDate && date && endDate <= date) setEndDate(null); };
  const handleEndDateChange = (date) => setEndDate(date);
  const handleAdultsChange = (e) => setAdults(Math.max(1, parseInt(e.target.value, 10) || 1));
  const handleChildrenChange = (e) => setChildren(Math.max(0, parseInt(e.target.value, 10) || 0));
  const handleHouseChange = (e) => { if (e.target.value && e.target.value !== houseId) { navigate(`/details/${e.target.value}`); } };

  useEffect(() => {
    if (!houseId) { setError(t('houseIdMissing')); setIsLoading(false); return; }
    let isMounted = true;
    const loadPageData = async () => {
      setIsLoading(true); setIsListLoading(true); setError(null);
      setHouseDetails(null); setAllHouses([]); setStartDate(null); setEndDate(null);
      setAdults(1); setChildren(0); setName(''); setEmail(''); setPhone(''); setMessage('');
      setRecaptchaToken(null); setFormErrors({});
      try {
         const [detailsData, listData] = await Promise.all([fetchPublicHouseDetails(houseId), fetchPublicHouses()]);
         if (!detailsData || !detailsData.id) throw new Error(t('invalidHouseData'));
         if (!listData) throw new Error(t('houseListFetchError'));
         if (isMounted) { setHouseDetails(detailsData); setAllHouses(listData); loadBookedDates(); }
      } catch (fetchError) {
  ...
  }, [houseId, t, loadBookedDates, navigate]);

  const handleLightboxViewChange = useCallback(({ index: currentIndex }) => { setLightboxIndex(currentIndex); }, []);

  if (isLoading) return <div className={styles.pageContainer}><h1>{t('loading')}</h1></div>;
  if (error) return <div className={styles.pageContainer}><h1>{t('errorFetching')}</h1><p>{error}</p><Link to="/" className={styles.backLink}>{t('backToHome')}</Link></div>;
  if (!houseDetails) return <div className={styles.pageContainer}><h1>{t('houseNotFound')}</h1><Link to="/" className={styles.backLink}>{t('backToHome')}</Link></div>;
    ...
    <div className={styles.detailsPageContainer}>
      ...
      <div className={styles.carouselSection}>
        ...
        {houseDetails.images?.length > 0 ? (
          <Swiper
              modules={[Navigation, Pagination, A11y, Autoplay]} spaceBetween={-10} slidesPerView={'auto'} centeredSlides={true} loop={true}
              navigation={true} pagination={{ clickable: true }} a11y={{ enabled: true }} className={styles.previewSwiper}
      ...
      {houseDetails.images?.length > 0 && (
          <Lightbox open={lightboxOpen} close={() => setLightboxOpen(false)} slides={houseDetails.images.map(src => ({ src }))} index={lightboxIndex} on={{ view: handleLightboxViewChange }} plugins={[Zoom]} zoom={{ maxZoomPixelRatio: 5, }}/>
      )}
          ...
          <form className={styles.bookingForm} onSubmit={handleSubmit}>
              ...
              <div className="control has-icons-left">
                <div className="select is-fullwidth">
                  <select id={`house-select-${houseId}`} value={houseId} onChange={handleHouseChange}>
                    ...
                    {allHouses.filter(h => h.id !== houseId).map(h => (<option key={h.id} value={h.id}>{h.name}</option>))}
            ...
            <div className="field">
             ...
             <div className="field">
                ...
                <div className="control"><div className={`select is-fullwidth ${formErrors.adults ? 'is-danger' : ''}`}><select id={`adults-${houseId}`} name="adults" value={adults} onChange={handleAdultsChange}>{Array.from({ length: 10 }, (_, i) => i + 1).map(num => (<option key={`adult-${num}`} value={num}>{num}</option>))}</select></div></div>
             ...
             <div className="field">
                ...
                <div className="control"><div className="select is-fullwidth"><select id={`children-${houseId}`} name="children" value={children} onChange={handleChildrenChange}>{Array.from({ length: 6 }, (_, i) => i).map(num => (<option key={`child-${num}`} value={num}>{num}</option>))}</select></div></div>
             </div>
             <div className="field">
                <label className="label" htmlFor={`message-${houseId}`}>{t('formMessage')}</label>
                <div className="control"><textarea className="textarea" id={`message-${houseId}`} name="message" placeholder={t('formMessagePlaceholder')} rows="4" value={message} onChange={handleMessageChange}></textarea></div>
             </div>
